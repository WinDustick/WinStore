...
BEGIN
  ...
  IF SUBSTR(v_version, 1, 2) >= '12' THEN
    ...
    IF v_count = 0 THEN
      ...
      DBMS_OUTPUT.PUT_LINE('Oracle Unified Audit policy created and enabled.');
    ELSE
      DBMS_OUTPUT.PUT_LINE('Oracle Unified Audit policy already exists.');
    END IF;
  ELSE
...
-- =====================================================================
-- Create generic audit trigger template procedure
-- =====================================================================
-- This is a PL/SQL code template for creating audit triggers
-- Do not execute this block directly, use it as a reference when creating triggers
-- for specific tables in the database
-- =====================================================================
/*
-- Copy and customize this template for each table requiring audit:

CREATE OR REPLACE TRIGGER TRG_AUDIT_[TABLE_NAME]
...
DECLARE
  ...
  v_user_id NUMBER;
  v_audit_id NUMBER;
  v_context NVARCHAR2(2000) := '[TABLE_NAME] audit'; -- Business context
BEGIN
  -- Determine operation type
  IF INSERTING THEN
    v_operation := 'INSERT';
    v_record_id := TO_CHAR(:NEW.[PRIMARY_KEY_COLUMN]);
  ELSIF UPDATING THEN
    v_operation := 'UPDATE';
    v_record_id := TO_CHAR(:OLD.[PRIMARY_KEY_COLUMN]);
  ELSIF DELETING THEN
    v_operation := 'DELETE';
    v_record_id := TO_CHAR(:OLD.[PRIMARY_KEY_COLUMN]);
  END IF;
  
  -- Get user ID - customize based on your table structure
  -- If your table has user_ID column:
  -- v_user_id := NVL2(:NEW.user_ID, :NEW.user_ID, :OLD.user_ID);
  
  -- OR if you need to get it from related tables:
  -- BEGIN
  --   SELECT user_ID INTO v_user_id 
  --   FROM Related_Table 
  --   WHERE related_id = NVL(:NEW.related_id, :OLD.related_id);
  -- EXCEPTION
  ...
  sp_LogBusinessAuditEvent(
    ...
    p_BusinessContext => v_context,
    p_AuditID => v_audit_id
  );
  
  -- Add specific column auditing for UPDATE operations if needed
  -- IF UPDATING THEN
  --   -- Log status changes
  --   IF :OLD.[STATUS_COLUMN] != :NEW.[STATUS_COLUMN] OR 
  --      (:OLD.[STATUS_COLUMN] IS NULL AND :NEW.[STATUS_COLUMN] IS NOT NULL) OR
  --      (:OLD.[STATUS_COLUMN] IS NOT NULL AND :NEW.[STATUS_COLUMN] IS NULL) THEN
  --     
  --     sp_LogBusinessAuditEvent(
  --       p_UserID => v_user_id,
  --       p_TableName => '[TABLE_NAME]',
  --       p_Operation => 'UPDATE',
  --       p_RecordID => v_record_id,
  --       p_ColumnName => '[STATUS_COLUMN]',
  --       p_OldValue => TO_CHAR(:OLD.[STATUS_COLUMN]),
  --       p_NewValue => TO_CHAR(:NEW.[STATUS_COLUMN]),
  --       p_BusinessContext => '[STATUS_COLUMN] change',
  --       p_AuditID => v_audit_id
  --     );
  --   END IF;
  -- END IF;
